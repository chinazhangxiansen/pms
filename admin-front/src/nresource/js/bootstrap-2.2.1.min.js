...
$(function(){
  ...
  /*信息展示树形列表*/
	$('.list-tree').on('click','.title',function () {
	    var parent = $(this).parent();
	    var muse = parent.children('ul');
	    muse.slideToggle(200,function () {
	        parent.toggleClass('open');
	        crm.resetIframeHeight();
	    });
	});
	...
	$(".switch").on('click', function(event){
		...
		var me = $(this);
		if (me.hasClass("switch-on")) {
			me.removeClass('switch-on').addClass('switch-off');
			me.text('关');
		}else{
			me.removeClass('switch-off').addClass('switch-on');
			me.text('开');
		}
	});

	/*panel-开关*/
	$(".panel-switch").on('click', function(event) {
		event.preventDefault();
		var me = $(this);
		if (me.hasClass("open")) {
			me.removeClass('open');
			me.closest('.panel').find('.panel-body').removeClass('none');
		}else{
			me.addClass('open');
			me.closest('.panel').find('.panel-body').addClass('none');
		}
		crm.resetIframeHeight();
	});
...
var crm = {
	...
	init:function(){
		var me = this;
		//关闭Modal
		$('.J_modalClose').on('click',function() {
	        var the = $(this),
	        modal = the.closest('.crm-modal'); 
	        modal.addClass("hide");
	        me.removeModalBg();
	    });
	},
	createModalBg : function(){
		var me = this;
		var	modalBg = me.d.createElement('div');
			modalBg.className = 'crm-modal-bg';
			modalBg.id = 'J_crmModalBg';
			modalBg.style.height = (me.d.body.scrollHeight || me.d.documentElement.scrollHeight) +"px";
			me.d.body.appendChild(modalBg);
		if(window != window.parent){
            parent.CRM.createModalBg(216);
        } 
	},
	removeModalBg : function(){
		var me = this,
			crmModalBg = me.d.getElementById('J_crmModalBg');
			if(crmModalBg) me.d.body.removeChild(crmModalBg); 
		if(window != window.parent){
            parent.CRM.removeModalBg();
        }
	},
	showModal : function(modalObj){//参数：jq对象
		var me = this;
		if(window == window.parent){//是否有父窗体
			var parentWinscrollTop = me.d.body.scrollTop || me.d.documentElement.scrollTop;
			modalObj.removeClass("hide").css('top', (parentWinscrollTop + 30) + 'px');
		}else{
			var parentWinscrollTop = parent.document.body.scrollTop || parent.document.documentElement.scrollTop,
				parentWinHeight = parent.document.body.scrollHeight || parent.document.documentElement.scrollHeight,
				currWinHeight = me.d.body.scrollHeight || me.d.documentElement.scrollHeight;
			if(parentWinscrollTop < 216){
				modalObj.removeClass("hide").css('top', '0');
			}else{
				modalObj.removeClass("hide");
				if((modalObj.height() + parentWinscrollTop) > (parentWinHeight-216)){//判断modal弹出后时候超出了底部
					modalObj.css('top', (currWinHeight - modalObj.height() - 30) + 'px');
				}else{
					modalObj.css('top', (parentWinscrollTop - 216 + 30) + 'px');
				}	
			}
		}
		me.createModalBg();
	},
	resetIframeHeight : function(){//重新设置当前iframe的高度
		if(window != window.parent){
			parent.CRM.setIframeHeight(window.frameElement);
		}
	}
}