...
function showMenu(ctrlid, align, click, offset, duration, timeout, layer, showid, maxh, drag) {
  ...
  if(is_ie && is_ie < 7) {
    if(!jsmenu['iframe'][layer]) {
      ...
      $('#' + 'append_parent')[0] ? $('#' + 'append_parent')[0].appendChild(iframe) : menuobj.parentNode.appendChild(iframe);
...
function hideMenu(layer) {
  ...
  if(jsmenu['active'][layer]) {
    try {
      $('#' + jsmenu['active'][layer].ctrlkey)[0].className = ctrlobjclassName;
    } catch(e) {}
...
function ShowImage(id,postfix)
{
   ...
   for(var i=0;i<inputs.length; i++)
   {
      ...
      $('#' + id+'_menu')[0].innerHTML += '<a href="javascript:InsertImage(\''+('file://'+(input.value.replace(a,"/")))+'\');" title="插入正文 '+input.value+'">'+fileName+'</a>';
   }
   ...
   for(var i=0;i<links.length; i++)
   {
      ...
      $('#' + id+'_menu')[0].innerHTML += '<a href="'+link.href+'" title="'+link.title+'">'+link.title+'</a>';
   }
   if($('#' + id+'_menu')[0].innerHTML=="")
      $('#' + id+'_menu')[0].innerHTML="没有添加图片文件";
   showMenu(id,1);
}